...
class SVM:
    ...
    def compute_loss(self, x, y):
        ...
        returns : float
        """
        #w = (number of features, m)
        M2 = np.maximum(0, 2 - y * x.dot(self.w)) ** 2
        output = ((self.C * np.sum(self.w ** 2))/2) + np.mean(np.sum(M2, axis=1))
    ...
    def compute_gradient(self, x, y):
        ...
        minibatch = x.shape[0]
        t1 = self.C * self.w
        t2 = (-2 * x.T.dot(y * np.maximum(0, 2 - y * x.dot(self.w))))/minibatch
        output = t1 + t2
        return output
    ...
    def infer(self, x):
        ...
        """
        maxx = np.argmax(x.dot(self.w), axis=1)
        output = self.make_one_versus_all_labels(maxx, self.m)
        return output
...
def plot():
    ...
    train_l, train_ac, test_l, test_ac = [], [], [], []
    for c in [1, 5, 10]:
        ...
        train_l.append(train_losses)
        train_ac.append(train_accs)
        test_l.append(test_losses)
        test_ac.append(test_accs)


    t = np.arange(0, 200)

    plt.plot(t, train_l[0], 'r', label='C = ' + str(1))  # plotting t, a separately
    plt.plot(t, train_l[1], 'b', label='C = ' + str(5))  # plotting t, b separately
    plt.plot(t, train_l[2], 'g', label='C = ' + str(10))  # plotting t, c separately
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.title('train loss on different C values')
    plt.show()

    plt.plot(t, train_ac[0], 'r', label='C = ' + str(1))  # plotting t, a separately
    plt.plot(t, train_ac[1], 'b', label='C = ' + str(5))  # plotting t, b separately
    plt.plot(t, train_ac[2], 'g', label='C = ' + str(10))  # plotting t, c separately
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.legend()
    plt.title('train accuracy on different C values')
    plt.show()

    plt.plot(t, test_l[0], 'r', label='C = ' + str(1))  # plotting t, a separately
    plt.plot(t, test_l[1], 'b', label='C = ' + str(5))  # plotting t, b separately
    plt.plot(t, test_l[2], 'g', label='C = ' + str(10))  # plotting t, c separately
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.title('test loss on different C values')
    plt.show()

    plt.plot(t, test_ac[0], 'r', label='C = ' + str(1))  # plotting t, a separately
    plt.plot(t, test_ac[1], 'b', label='C = ' + str(5))  # plotting t, b separately
    plt.plot(t, test_ac[2], 'g', label='C = ' + str(10))  # plotting t, c separately
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.legend()
    plt.title('test accuracy on different C values')
    plt.show()
