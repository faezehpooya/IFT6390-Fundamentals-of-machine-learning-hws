...
Original file is located at
    https://colab.research.google.com/drive/123s9Cp5h2pCnz-GJj6zzP3bHyvX62IXt
"""
...
def steps_to_one(i):
    ...
    if i%2==0:
      return 1+steps_to_one(i/2)
    
    else:
      return 1+steps_to_one(3*i+1)

"""### [4] Find Bins (BONUS FOR IFT3395 - UNDERGRAD)
...
### [4] Histogrammes (BONUS POUR IFT3395 - BAC)
...
def find_bins(input_list, k):
    ...
    """
    h= len(input_list)*1.0/k
    min_num = min(input_list)
    out= [0.0 for i in range(k+2)]
    out[0] = min_num-h/2
...
### [6] Normalisation de Jeu de Données
...
def data_normalization(X):
    ...
    return np.insert(X,len(data),label, axis=1)

"""### [7] Entropy of a Valid Discrete Probability Distribution
...
### [7] Entropie d'Une Distribution de Probabilité Discrète Valide
...
"""### [8] Heavyball Optimizer (BONUS FOR IFT3395 - UNDERGRADS)
...
### [8] Optimiseur de Heavyball (BONUS POUR IFT3395 - BAC)
...
def heavyball_optimizer(x, inputs, alpha=0.9, beta=0.1):
    ...
    new_X = x - alpha*inputs + beta*x
    return new_X

""" 
### [9] Machine Learning : Nearest Centroid Classifier
...
### [9] Apprentissage Automatique : Classificateur du Centroïde le Plus Proche
...
class NearestCentroidClassifier:
    ...
    def fit(self, X, y):  # question A
        ...
        for classes in range(self.k):
          self.centroids[classes]=np.sum(X[y==classes, :], axis=0).reshape(self.centroids.shape[1],-1)/len(X[y==classes, :])
    ...
    def predict(self, X):  # question B
        ...
        """
        num_test = X.shape[0]
        counts = np.ones((num_test, self.k))
        classes_pred = np.zeros(num_test)
        max_distance = 0
        # For each test datapoint
        for (i, ex) in enumerate(X):
            distances = self.minkowski_mat(ex, self.centroids) 
            neareset_IDX= np.argsort(distances)[0]
            classes_pred[i] = neareset_IDX
        return classes_pred
